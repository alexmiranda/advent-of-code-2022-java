...
public class Day21 {
    ...
    static final class MathJob implements Job {
        ...
        BigDecimal invert(Context ctx, boolean leftSide, BigDecimal target) {
            ...
            return switch (operator) {
                ...
                case '-' -> leftSide ? constant.add(target) : constant.subtract(target);
                case '/' -> leftSide ? constant.multiply(target) : constant.divide(target);
                default -> invalidOperator();
            };
        }
    ...
    static class Context {
        ...
        BigDecimal solve(String monkey) {
            return monkeyJobs.get(monkey).shout(this);
        }

        BigDecimal solvePart2(String monkey) {
            var humn = (HumanJob) monkeyJobs.get(monkey);
            return humn.shout(this);
        }
    }
